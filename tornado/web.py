...
from tornado.log import access_log, app_log, gen_log, ajax_log
...
class RequestHandler(object):
    ...
    def ajax(self, code, detail=''):
        """使用JSON格式返回信息
        code <  1000, detail仅在日志输出
        code >= 1000, detail仅在响应包中
        """
        if code < 1000:
            ajax_log.warning('%(code)d %(detail)s' % {
                'code': code,
                'detail': detail
            })
        else:
